...
class KnowledgeManager {
  ...
  async getByType(type: 'text' | 'choice' | 'code'): Promise<KnowledgeData[]> {
    const allKnowledge = await this.client.get<KnowledgeData[]>('/api/test/knowledge')
    return allKnowledge.filter(k => k.type === type)
  }

  async getAll(): Promise<KnowledgeData[]> {
    return this.client.get<KnowledgeData[]>('/api/test/knowledge')
  }
}
...
class TextQuestionManager {
  ...
  constructor(client: MemorinaAPIClient, knowledgeManager: KnowledgeManager) {
    this.client = client
  ...
  async create(input: {
    ...
    return this.client.post<TextQuestionData>('/api/test/textquestions', requestData)
  }
